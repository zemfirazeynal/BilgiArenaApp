...
class SignUpViewController: UIViewController {
    ...
    fileprivate func configureActions() {
        emailButton.addTarget(
            self,
            action: #selector(handleEmailSignup),
            for: .touchUpInside
        )

        loginLabel.addGestureRecognizer(
    ...
    private func configureLayout() {
        [
            navigationHeader, titleLabel, emailButton, googleButton, loginLabel,
            termsLabel,
        ].forEach {
            view.addSubview($0)
        }
        ...
        NSLayoutConstraint.activate([
            ...
            googleButton.topAnchor.constraint(
                equalTo: emailButton.bottomAnchor,
                constant: 20
            ),
            googleButton.leadingAnchor.constraint(
                equalTo: emailButton.leadingAnchor
            ),
            googleButton.trailingAnchor.constraint(
                equalTo: emailButton.trailingAnchor
            ),
            googleButton.heightAnchor.constraint(
                equalTo: emailButton.heightAnchor
            ),
